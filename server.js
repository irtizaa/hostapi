...
function getDataById(req, res) {
  ...
  conn
    ...
    .then(function () {
      let req = new sql.Request(conn);
      req
        .query(`SELECT * FROM ZONGOSP WHERE ID = ${userId}`)
        .then(function (result) {
...
function updateData(req, res) {
  ...
  conn
    ...
    .catch(function (err) {
      ...
      res
        .status(500)
        .send(`Failed to connect to the database. Error: ${err.message}`);
    });
}
...
function deleteDataById(req, res) {
  ...
  conn
    ...
    .then(function () {
      let req = new sql.Request(conn);
      req
        .query(`DELETE FROM ZONGOSP WHERE ID = ${userId}`)
        .then(function () {
